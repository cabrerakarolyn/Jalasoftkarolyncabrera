...
exports.translate_and_create_translation = (req, res) => {
  console.log("translate_and_create_translation")
  console.log(req)
  translator.translateAndCallback(req.body.original, this.create_a_translation, res)
}

exports.read_or_create_a_translation = (req, res) => {
  Translation.findOne({original: req.params.translationId}, (err, translation) => {
    if (err) res.send(err);
    if (translation) {
      res.json(translation)
    } else {
      console.log("read_or_create_a_translation")
  console.log(req)
      translator.translateAndCallback(req.params.translationId, this.create_a_translation, res)
    }
  })
};