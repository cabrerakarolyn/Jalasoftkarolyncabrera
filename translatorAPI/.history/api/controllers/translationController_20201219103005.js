...
exports.read_or_create_a_translation = (req, res) => {
  Translation.findOne(
    ...
    (err, translation) => {
      if (err) res.send(err);
      if (translation) {
        console.log("CREATED AT", translation.createdAt.getTime());
        console.log("CURRENT TIMESTAMP", Date.now().getTime())
        console.log("DIFF", Date.now().getTime() - translation.createdAt.getTime())
        res.json(translation);
      } else {
        translator.translateAndCallback(
