...
exports.read_or_create_a_translation = (req, res) => {
  ...
  Translation.findOne(
    ...
    (err, translation) => {
      if (err) res.send(err);
      console.log (Math.floor(Math.abs(Date.now() - translation.updatedAt.getTime()) / (60 * 60 * 10)))
      if (translation) {
        if (minutesDiff(Date.now(), translation.updatedAt.getTime()) <= 1) {
          res.json(translation);
        } else {
          console.log("ASDSAD");
          console.log(req.params.translationId)
          this.update_a_translation(req, res)
        }
      } else {
        translator.translateAndCallback(
