...
exports.read_or_create_a_translation = (req, res) => {
  Translation.findOne(
    ...
    (err, translation) => {
      if (err) res.send(err);
      if (translation) {
        const minutesDiff = () => Math.floor(Math.abs(time1 - time2) / (60 * 60 * 10))
        ...
        console.log("DIFF", (Date.now() - translation.createdAt.getTime()) / (60 * 60 * 10))
        console.log("WELL DIFF", minutesDiff(Date.now(), translation.createdAt.getTime()))
        res.json(translation);
      } else {
        translator.translateAndCallback(
