...
exports.update_a_translation = (req, res) => {
  Translation.findOneAndUpdate(
    { _id: req.params.translationId || req.body.original },
    req.body,
    { new: true },
    (err, translation) => {
      if (err) res.send(err);
...
exports.read_or_create_a_translation = (req, res) => {
  ...
  Translation.findOne(
    ...
    (err, translation) => {
      if (err) res.send(err);
      console.log (minutesDiff(Date.now(), translation.createdAt.getTime()))
      if (translation && minutesDiff(Date.now(), translation.createdAt.getTime()) <= 1) {
        res.json(translation);
      } else {
        translator.translateAndCallback(
          req.params.translationId,
          this.update_a_translation,
          res
        );
      }
    }
